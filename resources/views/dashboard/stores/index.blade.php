<x-dashboard-layout>
                ...
                <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                    ...
                    <thead>
                        <tr class="fw-bolder text-muted">
                            ...
                            <th class="min-w-120px">Owner Name</th>
                            <th class="min-w-120px">Mobile Number</th>
                            <th class="min-w-120px">Store Type</th>
                            <th class="min-w-120px">`Branch</th>
                        ...
                        <tr>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input status-toggle" type="checkbox"
                                            id="statusToggle_{{ $store->id }}" data-store-id="{{ $store->id }}" {{
                                            $store->status == 0 ? 'checked' : '' }}>
                                        <label class="form-check-label fw-bold text-gray-400 ms-3"
                                            for="statusToggle_{{ $store->id }}"></label>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                ...
                                <a href="#" class="btn btn-icon btn-danger btn-sm mx-1"
                                    onclick="showDeleteConfirmation({{ $store->id }})">
    ...
    <script>
        function showDeleteConfirmation(cityId) {
            ...
            deleteForm.action = "{{ route('stores.destroy', '') }}" + '/' + cityId; // Set the correct delete route
...
<script>
    $(document).ready(function () {
        $(document).on('change', '.status-toggle', function () {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route('update-store-status') }}', // Updated route name
                data: {
                    store_id: storeId,
                    status: newStatus
                },
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        $('#statusText_' + storeId).removeClass('text-success text-danger');
                        $('#statusText_' + storeId).addClass(newStatus === 0 ? 'text-success' : 'text-danger');
                        $('#statusText_' + storeId).text(newStatus === 0 ? 'Active' : 'Inactive');
                    } else {

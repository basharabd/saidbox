<x-dashboard-layout>
                ...
                <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                    ...
                    <thead>
                        <tr class="fw-bolder text-muted">
                            ...
                            <th class="min-w-120px">Branch Name</th>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="form-check form-switch form-check-custom form-check-solid">
                                        <input class="form-check-input status-toggle" type="checkbox"
                                            id="statusToggle_{{ $branch->id }}" data-branch-id="{{ $branch->id }}" {{
                                            $branch->status == 0 ? 'checked' : '' }}>
                                        <label class="form-check-label fw-bold text-gray-400 ms-3"
                                            for="statusToggle_{{ $branch->id }}"></label>
                                    </div>
                                </div>
                            </td>
    ...
    <script>
        function showDeleteConfirmation(cityId) {
            ...
            deleteForm.action = "{{ route('branches.destroy', '') }}" + '/' + cityId; // Set the correct delete route
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        $('#statusText_' + branchId).removeClass('text-success text-danger');
                        $('#statusText_' + branchId).addClass(newStatus === 0 ? 'text-success' : 'text-danger');
                        $('#statusText_' + branchId).text(newStatus === 0 ? 'Active' : 'Inactive');
                    } else {
